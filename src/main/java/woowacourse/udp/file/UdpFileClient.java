...
public class UdpFileClient {
    ...
    public UdpFileClient() {
        System.out.println("UDP File 클라이언트 초기화.");
    }
    ...
    public void start(
        ...
        try (
            ...
            while (true) {
                ...
                Path filePath = Path.of(BASE_PATH + SCANNER.nextLine());

                // 서버에 메시지를 보내기 위해서는 데이터를 바이트 배열로 변환해야 한다
                byte[] dateToSend = readBytesFromFilePath(filePath);

                // DatagramPacket 객체를 생성하여 서버에 데이터를 보낸다
                ...
                // 서버부터 메시지를 수신한다
                // 해당 메서드가 호출되면, 서버가 데이터를 보낼 때까지 블록된다
